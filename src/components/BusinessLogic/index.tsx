...
const BusinessLogic: React.FC<Props> = ({code, replaySlug, roomName}) => {
  ...
  const addUserScore = async (replay: Replay) => {
    const data:any = await addScore({replay, roomId});
    setUserReplay(null);
  ...
  const getLeaderboardData = async () => {
    ...
    const data :any = await getLeaderboard(roomId);
    setLeaderboard(data)
    setLoading(null);
  }
  ...
  const getReplayData = async (id: number) => {
    ...
    const data: any = await getReplayById(id);
  ...
  const getUserRunsData = async ({next, limit, userId} : {next?: string, limit?: number, userId: number, roomId?: number}) => {
    ...
    const data : any= await getUserRuns({next, limit, userId: userId, roomId});
    if (next) {
  ...
  const getSharedReplayData = async (slug: string, roomId?: number) => {
    try {
      setLoading('shared');
      const data:any = await getSharedReplay(slug, roomId);
  ...
  const getMyRanksData = async ({next, limit} : {next?: string, limit?: number}) => {
    ...
    const data:any = await getMyRanks({next, limit, roomId});
    if (next) {
  ...
  const getRoomData = async (roomName: string) => {
    try {
      const data:any = await getRoom(roomName);
      setRoomId(data.id)
      setIsRoomExist(true);
    } catch (e) {
